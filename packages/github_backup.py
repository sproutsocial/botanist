...
class Helpers(object):
    ...
    def https_url_with_auth(self, base_url):
        ...
        quoted_username = requests.utils.quote(self.args.username)
        quoted_password = requests.utils.quote(self.args.password)
        return f"https://{quoted_username}:{quoted_password}@{suffix}"
...
def get_repos(org, repo_type, access_token=None, username=None, password=None, per_page=25):
    ...
    """
    rq_params = {"params": {"type": repo_type, "per_page": per_page}}
    url = API_BASE + "orgs/%s/repos?" % org
    if access_token:
        rq_params["params"]["access_token"] = args.access_token
    elif username and password:
        rq_params["auth"] = (username, password)
    else:
        raise ValueError("unworkable combination of authentication inputs")
    rsp = requests.get(url, **rq_params)

    try:
        pagination = get_pagination(rsp.headers["Link"])
    except KeyError:
...
if __name__ == "__main__":
    ...
    ssh_parser = subparsers.add_parser(
        "ssh", help="use ssh for cloning/pulling, and use access token for github api access"
    )
    ssh_parser.add_argument(
        ...
        dest="directory",
        required=True,
        help="full or relative path to store backed up repositories",
    )
    ...
    ssh_parser.add_argument(
        ...
        help="repository types to backup",
    )
    ssh_parser.add_argument("-a", "--access-token", type=str, help="personal access token or oauth access token")
    ssh_parser.add_argument(
        "-f", "--forks", action="store_true", help="add this arg if you want to backup fork repositories also"
    )
    ssh_parser.add_argument(
        "-i",
        "--ignore-list",
        type=repocsv,
        default=set(),
        help="add repos you dont want to fetch/index, e.g. --ignore-list org1/repo1,org2/repo2",
    )
    ...
    https_parser = subparsers.add_parser(
        "https",
        help="use https for cloning/pulling, and use username and password (https basic auth) for github api access. note that github also allows using a personal access token as a password via this method",
    )
    ...
    https_parser.add_argument(
        ...
        type=str,
        required=True,
        help="github password or github personal access token",
    )
    https_parser.add_argument(
        "-f", "--forks", action="store_true", help="add this arg if you want to backup fork repositories also"
    )
