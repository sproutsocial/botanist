...
def do_search(query, case_sensitive=True) -> str:
    cmd = [path.join(BIN_PATH, "csearch"), "-n"]
    if not case_sensitive:
        cmd.append("-i")
    cmd.append(query)
    log.info("cmd = %s", cmd)

    p = Popen(cmd, stdout=PIPE, stderr=PIPE, text=True, encoding="UTF-8")
    ...
    for line in lines:
        ...
        try:
            ...
            if repo_type == "git" and fully_qualified_repo_name not in default_git_branches:
                ...
                cmd = ["git", "--git-dir", git_dir, "branch"]
                p = Popen(cmd, stdout=PIPE, stderr=PIPE, text=True, encoding="UTF-8")
                out, err = p.communicate()
                if p.returncode == 0:
            ...
            result = {
                ...
                "deeplink": deep_link(
                    vcs_loc,
                    fully_qualified_repo_name,
                    filename,
                    repo_type,
                    lineno,
                    default_git_branches.get(fully_qualified_repo_name, None),
                ),
                "count": count,
            }
